...
-- Gecorrigeerde versie met juiste kolom namen
...
-- Probeer verschillende kolom namen
INSERT INTO product_sizes (product_id, size, stock_type, stock_level)
SELECT 
    p.id,
    unnest(p.sizes) as size,
    COALESCE(p."stockType", p.stocktype) as stock_type,
    COALESCE(p."stockLevel", p.stocklevel, 0) as stock_level
FROM products p
