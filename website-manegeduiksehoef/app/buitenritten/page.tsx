...
const BLOCKED_BUITENRIT_DATES: { year: number; month: number; day: number }[] = [
  ...
  { year: 2026, month: 3, day: 26 }, // 26 april 2026
...
export default function BuitenrittenPage() {
  ...
  // Aankondigingsbanner: vanaf 7 september t/m 1 april (kalender winterstop blijft 19 okt t/m 1 april)
  const showWinterStopBanner = (() => {
    const now = new Date()
    const y = now.getFullYear()
    const announcementStart = new Date(y, 8, 7) // 7 september
    const winterEnd = new Date(y + 1, 3, 1, 23, 59, 59, 999) // 1 april volgend jaar
    if (now >= announcementStart) return now <= winterEnd
    const prevStart = new Date(y - 1, 8, 7)
    const thisEnd = new Date(y, 3, 1, 23, 59, 59, 999)
    return now >= prevStart && now <= thisEnd
  })()
              ...
              <div className="flex items-start md:items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm md:text-base text-white/80 leading-relaxed max-w-3xl">
                    Vanaf <strong className="text-white">19 oktober</strong> tot en met <strong className="text-white">1 april</strong> zijn onze buitenritten gesloten vanwege onvoorspelbare weeromstandigheden.
                    Wil je in deze periode toch graag een buitenrit boeken? Neem dan gerust contact met ons op via{' '}
                    <a href="mailto:info@manegeduiksehoef.nl" className="text-white font-semibold underline underline-offset-2 hover:text-cyan-200">
                      e-mail
                    </a>{' '}
